          ...
          <div className="flex items-start justify-between">
            <div>
              <div className="flex items-center gap-2 mb-1">
                <Brain className="w-5 h-5 text-drc-green" />
                <span className="text-drc-green text-sm font-semibold">CDP-AI OS — Agent de Sélection</span>
              </div>
                ...
                {['all', 'National', 'Provincial', 'Local'].map(level => (
                  <button
                    ...
                    className={`text-xs px-2 py-1 rounded-full font-medium transition-all ${
                      filterLevel === level ? 'bg-drc-green text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'
                    }`}
                  >
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 rounded-lg text-sm transition-all ${
                      selectedRole.id === role.id
                        ? 'bg-drc-green text-white font-semibold'
                        : 'text-gray-700 hover:bg-gray-100'
                    }`}
                  >
                    <p className="font-medium leading-tight">{role.label}</p>
                    <p className={`text-xs mt-0.5 ${selectedRole.id === role.id ? 'text-green-200' : 'text-gray-400'}`}>
                      Niveau {role.level}
                    </p>
                  </button>
                ))}
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded-lg">
                ...
                {[
                  ...
                  <div key={item.label} className="flex items-center justify-between mb-1">
                    ...
                    <span className="text-xs font-bold text-drc-green">{item.weight}%</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="lg:col-span-3 space-y-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="font-bold text-gray-900">
                Top 3 — <span className="text-drc-green">{selectedRole.label}</span>
              </h2>
